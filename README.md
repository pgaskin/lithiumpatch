...
5. Optionally download additional fonts into the `fonts` direcotry to add additional fonts (to limit them to a single language, put them in a subdirectory named `latin`/`cyrillic`/`greek`/`thai`).
